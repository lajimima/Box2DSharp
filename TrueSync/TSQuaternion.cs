...
namespace TrueSync
{
    ...
    public struct TSQuaternion
    {
        ...
        {
            angle = FP.FastMod(angle, 360);
            if (FP.FastAbs(angle) < FP.Epsilon)
            {
                angle = FP.Zero;
            }
            else if (angle < FP.Zero)
            {
                angle += 360;
            }
            return angle;
        ...
        {
            return CreateFromMatrix(TSMatrix.LookAt(forward));
        }
        ...
        public static TSQuaternion CreateByYaw(FP yaw)
        {
            TSQuaternion rotation;
            FP num = yaw * FP.HalfDeg2Rad;

            rotation.x = FP.Zero;
            rotation.y = FP.Sin(num);
            rotation.z = FP.Zero;
            rotation.w = FP.Cos(num);
        ...
        public static TSQuaternion Lerp(TSQuaternion from, TSQuaternion to, FP t)
        {
        ...
        {
            FP num = quat.x + quat.x;
            FP num2 = quat.y + quat.y;
            FP num3 = quat.z + quat.z;
            FP num4 = quat.x * num;
            FP num5 = quat.y * num2;
            FP num6 = quat.z * num3;
            FP num7 = quat.x * num2;
